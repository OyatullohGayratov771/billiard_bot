...
func (r *UserRepo) SavePhone(tgID int64, phone string) error {
	_, err := r.db.Exec(`UPDATE users SET phone = $1 WHERE telegram_id = $2`, phone, tgID)
	return err
}

func (r *UserRepo) UpdateName(tgID int64, firstName string) error {
	_, err := r.db.Exec(
		`UPDATE users SET first_name = $1, name_customized = true WHERE telegram_id = $2`,
		firstName, tgID,
	)
	return err
}
