...
// Jarayon: ISAPI search (kesh yuklash) → RTSP -c copy (original sifat, decode yo'q)
func (r *Recorder) RecordFromNVR(clipID int64, nvrHost, nvrUser, nvrPass string, nvrPort, channel int, startTime, endTime time.Time) (string, error) {
	...
	// NVR keshini yuklash — RTSP dan oldin kerak
	...
	log.Printf("[klip#%d] RTSP: %s", clipID, rtspURL)

	// timeout = klip uzunligi * 2 + 2 daqiqa (NVR seek + tarmoq bufer uchun)
